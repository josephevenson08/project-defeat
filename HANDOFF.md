...
npm run lint                      # exit 0
npm run build
npx playwright test --reporter=line   # 58 passing. Use --reporter=line, not the default.
...
### 1.6 What the catalog audit has actually revealed
Four batches in (7 caster/healer weapons, 8 relics and school-restricted off-hands, 8 Tier 5 helms,
14 trinkets), the errors are not random and are worth understanding before doing more.

**The data was generated by inferring what an item *should* look like for its role.** That is why it
is uniformly plausible and frequently wrong in ways no amount of eyeballing would catch:

- Stats invented that the item simply does not carry — a parry rating, a Spirit, a dodge+parry pair,
  a base spell hit, and `mp5` on three separate healer trinkets that have none. That last one reads
  exactly like "healer trinket, add some mp5" applied as a template.
- Whole wrong stat *families*: a hit-rating DPS trinket recorded as a tank piece, a Rogue trinket
  given Agility and haste when it has hit rating and an **armor penetration** proc that had been
  written up as a haste proc.
- Rift Stalker Helm had the wrong stat set entirely, because real TBC Hunter tier carried Intellect
  and mp5 a Hunter barely used. An estimate assumes sensible itemisation; TBC often wasn't.
...
socket colours across the other 62 socketed items are varied rather than role-uniform. So verify
