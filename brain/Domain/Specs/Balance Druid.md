---
type: spec
class: Druid
spec: Balance
role: Caster DPS
generated: true
tags: [brain/domain, domain/spec, role/Caster-DPS]
---

# Balance Druid

[[Druid]] · [[Caster DPS]]

## Signature ability

**Starfire** (rank 8) — spell ID 26986, Direct Damage

- Cast: 3.5s · GCD 1.5s
- Base amount: 550–647
- Spell power coefficient: 1 (basis: castTime/3.5)
- Cost: 370 Mana

> Balance is a Starfire-spam spec with Moonfire and Insect Swarm maintained alongside it, and Wrath (rank 10, spell 26985, 255 mana, 2.0s cast, 383-432, 0.5714 coefficient) used as a faster filler when Nature's Grace is up or mana is tight. Talents shorten the cast (Starlight Wrath, -0.1s per point, up to -0.5s) without reducing the coefficient, so the 3.5s figure here is the untalented base and a real Balance druid casts it at 3.0s.

## Best in slot

- **Phase 2** — 58 ranked entries across 15 slots · source: Balance Druid DPS Best in Slot (BiS) Phase 2 Gear Guide



Sends you to: [[Gruul's Lair]], [[Karazhan]], [[Magtheridon's Lair]], [[Serpentshrine Cavern]], [[Tempest Keep - The Eye]]

## Where this lives in the code

- [[domain.bis.bisLists]] — `src/domain/bis/bisLists.ts`
- [[domain.abilities.sampleSignatureAbilities]] — `src/domain/abilities/sampleSignatureAbilities.ts`

## Related

- [[Best in Slot]]
- [[Signature Abilities]]
- [[Stat Weights]]
- [[Needs Verification]]

Up: [[TBC Knowledge Map]]

<!-- brain:manual -->

## Notes

_Anything you write below the marker above is kept when the brain is regenerated._
