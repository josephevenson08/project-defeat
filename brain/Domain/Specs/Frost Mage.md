---
type: spec
class: Mage
spec: Frost
role: Caster DPS
generated: true
tags: [brain/domain, domain/spec, role/Caster-DPS]
---

# Frost Mage

[[Mage]] · [[Caster DPS]]

## Signature ability

**Frostbolt** (rank 13) — spell ID 27072, Direct Damage

- Cast: 3s · GCD 1.5s
- Base amount: 600–647
- Spell power coefficient: 0.8143 (basis: castTime/3.5)
- Cost: 330 Mana

> Frost is a Frostbolt-spam spec and is the cleanest fit of any caster for a single-ability model. Frost was not a competitive raid DPS spec in Phase 1/2 (it is generally an Arcane or Fire mage that is brought), so accuracy here matters mostly for completeness. Frostbolt is a binary spell for resistance purposes, which is why Elemental Precision gives it double hit value.

## Best in slot

- **Phase 2** — 61 ranked entries across 15 slots · source: Frost Mage DPS Best in Slot (BiS) Phase 2 Gear Guide



Sends you to: [[Gruul's Lair]], [[Karazhan]], [[Magtheridon's Lair]], [[Serpentshrine Cavern]], [[Tempest Keep - The Eye]]

## Where this lives in the code

- [[domain.bis.bisLists]] — `src/domain/bis/bisLists.ts`
- [[domain.abilities.sampleSignatureAbilities]] — `src/domain/abilities/sampleSignatureAbilities.ts`

## Related

- [[Best in Slot]]
- [[Signature Abilities]]
- [[Stat Weights]]
- [[Needs Verification]]

Up: [[TBC Knowledge Map]]

<!-- brain:manual -->

## Notes

_Anything you write below the marker above is kept when the brain is regenerated._
