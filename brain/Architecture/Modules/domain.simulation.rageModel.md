...
`src/domain/simulation/rageModel.ts` · **domain** layer · 207 lines
...
**function** — `bloodrageRagePerSecond`, `rageDumpUsesPerSecond`, `rageFromDamageTaken`, `rageFromOneSwing`, `ragePerSecondFromWeapon`

**const** — `BLOODRAGE_COOLDOWN_SECONDS`, `BLOODRAGE_INSTANT_RAGE`, `BLOODRAGE_RAGE_PER_IMPROVED_RANK`, `BLOODRAGE_RAGE_PER_TICK`, `BLOODRAGE_TICKS`, `HEROIC_STRIKE_SUPPRESSES_SWING_RAGE`, `MAIN_HAND_HIT_FACTOR`, `MAX_RAGE`, `OFF_HAND_HIT_FACTOR`, `RAGE_CONVERSION_FACTOR`, `RAGE_MULTIPLIER_UNTALENTED`, `RAGE_PER_POINT_OF_DAMAGE`, `RAGE_PER_POINT_OF_DAMAGE_TAKEN`
