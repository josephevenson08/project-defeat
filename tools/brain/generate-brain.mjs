...
const PHASES = [
  ...
  },
  {
    ...
    modules: [
      ...
      'domain/professions/characterProfessions.ts',
      'domain/enchants/sampleEnchants.ts',
      'features/buffs/BuffsPanel.tsx',
  ...
  },
  {
    ...
    status: 'partial',
    summary: 'Upgrade planning, side-by-side gear comparison, named saves, import/export, shareable build links, source-and-cost planning and a phone layout all work. There is no autosave, by design, and no account or cloud copy.',
    done: [
    ...
    modules: [
      ...
      'features/gear/compareItems.ts',
      'features/gear/ComparePanel.tsx',
      'domain/builds/buildSerialization.ts',
      'domain/builds/shareLink.ts',
      'features/builds/ShareNotice.tsx',
...
async function writeProjectNotes(modules, counts) {
  ...
  await writeNote(
    ...
    [
      ...
      '',
      `1. **Multi-ability rotations** — started, and **5 specs of 27 press more than one button**: Arms and Fury Warrior, Affliction and Destruction Warlock, and Shadow Priest. The other 22 are a single-ability approximation, and every spec still loses any special whose sustained rate is not computable (rage-costed abilities with no cooldown, and Steady Shot), so they remain understated by differing amounts. The engine handles arbitrary ability lists; what gates the rest is sourced ability data. Still the biggest accuracy gap in ${link('Phase 4 - Simulation')}.`,
