...
const PHASES = [
  ...
  },
  {
    ...
    status: 'partial',
    summary: 'Upgrade planning and the serialization foundation exist. Save/load UI and comparison do not.',
    done: [
    ...
    modules: ['features/simulator/findUpgrades.ts', 'features/simulator/UpgradesPanel.tsx', 'domain/builds/buildSerialization.ts'],
  },
  {
    number: 6,
...
async function writeProjectNotes(modules, counts) {
  ...
  await writeNote(
    ...
    [
      ...
      '',
      `1. **Build save/load** — ${link('domain.builds.buildSerialization')} encodes and decodes a full build, and nothing calls it. Wiring it up closes the first ${link('Phase 5 - Planner Workflows')} item.`,
      `2. **Rotation modelling** — ${link('Signature Abilities')} now feed the caster and healer estimates, but the physical path still ignores them, so melee specs are white-damage-only. That is the biggest remaining accuracy gap in ${link('Phase 4 - Simulation')}.`,
      '',
      '## Related',
      '',
