...
import { getTargetDebuffById, sampleTargetDebuffs } from '../src/domain/buffs/sampleTargetDebuffs'
...
test('a debuff that cannot be modelled is listed in the panel without a checkbox', async ({ page }) => {
  ...
  const wintersChill = page.getByTestId('target-debuff-unmodelled-winters-chill')
  await expect(wintersChill).toBeVisible()
  await expect(wintersChill).toContainText('Frost')
  expect(await wintersChill.locator('input').count(), 'an unmodelled debuff must not offer a checkbox').toBe(0)

  // The modelled ones keep their checkbox, and read in flat armor rather than a percentage.
  const sunder = page.getByTestId('target-debuff-toggle-sunder-armor')
  await expect(sunder).toBeVisible()
  await expect(page.getByTestId('buffs-panel')).toContainText('-2600 target armor')
})
